...
scholar.appendRank = function () {
    ...
    elements.each(function () {
        ...
        let title = node.text();
        let data = $(this).find("div.gs_a").text().replace(/[\,\-\…]/g, "").split(" ");
        let author = data[1];
...
scholar.appendRanks = function () {
    ...
    elements.each(function () {
        ...
        if (!node.next().hasClass("ccf-rank")) {
            let title = node.text();
            let author = $(this).find("div.gs_gray").text().replace(/[\,\…]/g, "").split(" ")[1];
...
function fetchRank(node, title, author, year) {
    ...
    api_format = "https://dblp.org/search/publ/api?q=" + encodeURIComponent(title + " " + author + " year:" + year + ":") + "&format=json";
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
            ...
            if (resp["@total"] != 0) {
                ...
                dblp_url = url.substring(url.indexOf("/rec/") + 4, url.lastIndexOf("/"))
            } else { dblp_url == ""; };
